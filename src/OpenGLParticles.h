...
//  This file is part of SLAX, whose distribution is governed by the LICENSE
//  file.
// #####################################################################
...
public:
  ...
  void Set_Data_Pointers(const Array<Vector3> *_points,
                         const Array<real> *_colors = nullptr) {
    points = _points;
    colors = _colors;
  }
  ...
  virtual void Update_Data_To_Render() {
    ...
    for (size_type i = 0; i < (*points).size(); i++) {
      ...
      if (use_varying_point_size)
        OpenGL_Vertex(varying_point_size[i], opengl_vertices);
      else
        OpenGL_Vertex(placeholder, opengl_vertices); ////placeholder, 1 float
      if (use_vtx_color) {
    ...
    if (use_vtx_color) {
      Set_OpenGL_Vertex_Attribute(idx, 4, stride_size, idx * 4);
      idx++;
    } ////color
    Clear_OpenGL_Arrays();
  }
...
public:
  ...
  virtual void Initialize() {
    ...
    Initialize_Vector_Fields_Helper(particles);
  }

  void Set_Data_Pointers() { Set_Data_Pointers_Helper(particles); }

  virtual void Update_Data_To_Render() {
    if (!Update_Data_To_Render_Pre())
      return;

    Set_Data_Pointers();
    opengl_points.Update_Data_To_Render();
    for (auto &vf : opengl_vector_fields) {
      vf.Update_Data_To_Render();
    }
  ...
  virtual void Refresh(const int frame) {
    std::string file_name =
        output_dir + "/" + std::to_string(frame) + "/" + name;
  ...
  virtual void Set_Point_Size(const Array<GLfloat> &point_size) {
    opengl_points.varying_point_size = point_size;
    opengl_points.use_varying_point_size = true;
  }

protected:
  void Initialize_Vector_Fields_Helper(Particles<3> *particles = nullptr) {
    opengl_vector_fields.resize(2);
    for (auto &vf : opengl_vector_fields) {
      vf.Initialize();
    }
  }
  void Set_Data_Pointers_Helper(Particles<3> &particles) {
    ...
    opengl_vector_fields[0].visible = false;
    opengl_vector_fields[1].Set_Data_Pointers(particles.X(), particles.F());
    opengl_vector_fields[1].visible = false;
  } ////turn off visibility of vector fields by default
};
#endif
